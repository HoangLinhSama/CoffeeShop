...
class CheckSignedInRepositoryTest {
    ...
    @Test
    fun returnsStateSignIn_whenStateSignInIsSavedInDataStore() = runTest {
        whenever(preferences[PreferenceKey.LOGGED_IN]).thenReturn(true)
        whenever(userSettingDataStore.data).thenReturn(flowOf(preferences))

        repository.checkSignedIn().test {
